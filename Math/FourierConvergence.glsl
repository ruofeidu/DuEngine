...
const float duration = 10.0; // duration visualizing each wave in seconds.
const float transition = 3.0; // transition speed. (dont know the unit) 
...
const float freq = 0.5; // frequency in hz
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for(int i=0; i<terms; i++) {
        ...
        sum += wave(n, -uv.x+time+3.0, l, 0.0)*mul;
        dsum += dwave(n, -uv.x+time+3.0, l, 0.0)*mul;
        tsum += term;
    }
