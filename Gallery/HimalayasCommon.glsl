...
mat3 getCamera( in float time, inout vec3 ro, inout vec3 ta ) {
    ro = CAMERA_RO;
    ro.x += -cos(time*.13)*5.*INV_SCENE_SCALE;
    ro.z += (-cos(time*.1)*100.+20.)*INV_SCENE_SCALE;
    ta = ro + vec3(-.1,.15,1.)*200.*INV_SCENE_SCALE;
...
// 
vec4 saveCamera( in float time, in vec2 fragCoord ) {   
    vec3 ro, ta;
    mat3 cam = getCamera( time, ro, ta );
    vec4 fragColor;
    
    if( abs(fragCoord.x-3.5)<0.5 ) fragColor = vec4( cam[2], -dot(cam[2],ro) );
    if( abs(fragCoord.x-2.5)<0.5 ) fragColor = vec4( cam[1], -dot(cam[1],ro) );
    if( abs(fragCoord.x-1.5)<0.5 ) fragColor = vec4( cam[0], -dot(cam[0],ro) );
    
    return fragColor;
}

vec2 reprojectPos( in vec3 pos, in vec2 resolution, in sampler2D storage ) {
    mat4 oldCam = mat4( texelFetch(storage,ivec2(1,0),0),
                        texelFetch(storage,ivec2(2,0),0),
...
bool letterBox(vec2 fragCoord, const vec2 resolution, const float aspect) { 
    if( fragCoord.x < 0. || fragCoord.y >= resolution.x ||
