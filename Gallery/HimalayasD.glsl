...
#define CLOUDS_BASE_EDGE_SOFTNESS (.115)
...
float cloudMapDetail(vec3 p) { 
    // 3d lookup in 2d texture :( - no interpolation for now
    p *= (0.0016 * CLOUDS_BASE_SCALE * CLOUDS_DETAIL_SCALE);
    float zi = floor(p.z * (8./32.));
    ivec2 offset = ivec2(mod(floor(zi/8.),4.), mod(zi,8.))*32;
    return texelFetch(iChannel3, ivec2(mod(p.xy,32.))+offset, 0).r;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {            
    if (fragCoord.y < 1.5) {
        fragColor = saveCamera(iTime, fragCoord);
        if( abs(fragCoord.x-0.5)<0.5 ) fragColor = vec4(iResolution.xy,0,0);
    } else {
        if( letterBox(fragCoord, iResolution.xy, 2.25) ) {
            ...
            vec3 ro, rd;
    		getRay( iTime, fragCoord, iResolution.xy, ro, rd);
