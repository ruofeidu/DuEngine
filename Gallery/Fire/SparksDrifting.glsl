...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  //
  vec3 displacePos =
      vec3(1.0, 0.5, 1.0) * 2.4 * position + realTime * vec3(0.01, -0.7, 1.3);
  vec3 displace3 = vec3(noiseStackUV(displacePos, 2, 0.4, 0.1), 0.0);
  //
  vec3 noiseCoord =
      (vec3(2.0, 1.0, 1.0) * position + timing + 0.4 * displace3) / 1.0;
  ...
  // smoke
  float smokeNoise =
      0.5 + snoise(0.4 * position + timing * vec3(1.0, 1.0, 0.2)) / 2.0;
  ...
  sparkCoord -=
      30.0 * noiseStackUV(0.01 * vec3(sparkCoord, 30.0 * time), 1, 0.4, 0.1);
  ...
  float sparkLife =
      min(10.0 * (1.0 -
                  min((sparkGridIndex.y + (190.0 * realTime / sparkGridSize)) /
  ...
  if (sparkLife > 0.0) {
    ...
    float sparksGray =
        max(0.0, 1.0 - sparkLength / (sparkSize * sparkGridSize));
