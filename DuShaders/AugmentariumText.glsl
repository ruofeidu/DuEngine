// 
#define line1 T_ h_ e_
#define line2 A_ u_ g_ m_ e_ n_ t_ a_ r_ i_ u_ m_

// Thanks for Andre's awesome font: [url]https://www.shadertoy.com/view/4s3XDn[/url]
// For [url=http://augmentarium.umd.edu/]The Augmentarium[/url]
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float weight  = 0.01+us.w*min(iTime*.02-.03,0.03);//+.03*length(sin(uv*6.+.3*iTime));//+0.02-0.06*cos(iTime*.4+1.);
    if (ms>0.) {
