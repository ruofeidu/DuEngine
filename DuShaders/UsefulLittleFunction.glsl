...
float Gain(vec2 p) {
    return p.y - gain(p.x, 1.0);
}

float Gain2(vec2 p) {
    return p.y - gain(p.x, 2.0);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec2 uv = ( fragCoord.xy / iResolution.y ) - aspect / 4.0;
	uv *= DISP_SCALE;
	
	vec3 col = vec3(grid(uv) * 0.15);
    ...
    PLOT(Gain, RGBLabel(1), col, uv);
    PLOT(Gain2, RGBLabel(2), col, uv);
    PLOT(Gain3, RGBLabel(6), col, uv);
