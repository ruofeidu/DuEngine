...
float value_noise(vec3 p) {
  ...
  return mix(
      mix(mix(hash31(pi + vec3(0, 0, 0)), hash31(pi + vec3(1, 0, 0)), w.x),
...
float simplex_noise(vec3 p) {
  ...
  vec4 n =
      h * h * h * h * vec4(dot(d0, hash33(i)), dot(d1, hash33(i + i1)),
...
float noise_itself(vec3 p) { return noise(p * 8.0); }
...
float map(vec3 pos) {
  return length(pos - (vec3(0.0, 0.0, 1.5) + iTime * vec3(0.0, 0.0, SPEED))) -
         1.0;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec3 rd =
      vec3((p * 2.0 - 1.0) * vec2(iResolution.x / iResolution.y, 1.0), 1.0);
  col = marching(ro, rd, p, split);

  col = draw_circle_outline(p * vec2(iResolution.x / iResolution.y, 1.0), 0.9,
                            col);
  col =
      mix(vec3(0.3, 0.0, 0.0), col, smoothstep(0.0, 0.005, abs(p.x - split.x)));
  col = mix(vec3(0.3, 0.0, 0.0), col,
            smoothstep(0.0, 0.005 * iResolution.x / iResolution.y,
                       abs(p.y - split.y)));
