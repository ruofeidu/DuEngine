...
float truncf(float a, float l) { return floor(a * l) / l; }
...
float histogram(int iter, vec2 uv, vec2 interval, float height, float scale) {
  ...
  vec2 bucket = vec2(truncf(t, NUM_BUCKETS_F),
                     truncf(t, NUM_BUCKETS_F) + 1.0 / NUM_BUCKETS_F);
  float bucketval = 0.0;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // display lines
  if (abs(uv.x - 1.0 / 4.0) < 0.002)
    o = 0.0;
  if (abs(uv.x - 2.0 / 4.0) < 0.002)
    o = 0.0;
  if (abs(uv.x - 3.0 / 4.0) < 0.002)
    o = 0.0;
  if (abs(uv.y - 1.0 / 4.0) < 0.002)
    o = 0.0;
