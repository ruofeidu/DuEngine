...
float truncate(float a, float l) { return floor(a * l) / l; }
...
float n2rand_faster(vec2 n) {
  ...
  nrnd0 =
      max(-1.0,
          nrnd0); // Nerf the NaN generated by 0*rsqrt(0). Thanks @FioraAeterna!
...
float histogram(int iter, vec2 uv, vec2 interval, float height, float scale) {
  ...
  vec2 bucket = vec2(truncate(t, NUM_BUCKETS_F),
                     truncate(t, NUM_BUCKETS_F) + 1.0 / NUM_BUCKETS_F);
  float bucketval = 0.0;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (abs(uv.x - 1.0 / 5.0) < 0.002)
    o = 0.0;
  if (abs(uv.x - 2.0 / 5.0) < 0.002)
    o = 0.0;
  if (abs(uv.x - 3.0 / 5.0) < 0.002)
    o = 0.0;
  if (abs(uv.x - 4.0 / 5.0) < 0.002)
    o = 0.0;
