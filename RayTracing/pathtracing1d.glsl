...
#ifdef ANIMATENOISE
    float seed = p.x + fract(p.y * 18753.43121412313) + fract(12.12345314312*float(iFrame)/60.0);
#else
...
#ifdef MOTIONBLUR
        initLightSphere( float(iFrame)/60.0 + hash1(seed) / MOTIONBLURFPS );
#else
        initLightSphere( float(iFrame)/60.0 );        
#endif
