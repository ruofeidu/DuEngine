...
print(
    "Usage: ShaderName [NumOfBuffers] [NumOfChannelsOfMainBuffer] [ShaderToyID] // ~%d"
    % len(sys.argv))
filename = input(
    "Enter the name, or name followed by shader ID of the ShaderToy: ")
...
if re_shaderID.match(filename):
  ...
  url = "https://www.shadertoy.com/api/v1/shaders/%s?key=%s" % (shaderToyName,
                                                                APP_KEY)
  print(url)
  try:
...
print("%s created with %d channels and %d additional buffers." %
      (filename, numChannels, numBuffers))
