...
ShaderToy::ShaderToy(DuEngine* _renderer, double _width, double _height,
                     int _x0, double _y0) {
  auto geometry = new ShaderToyGeometry(_width, _height, _x0, _y0);
  auto numChannels =
      _renderer->m_config->GetIntWithDefault("channels_count", 0);
  m_screenBuffer = new ShaderToyScreenBuffer(geometry, numChannels);

  auto buffers_count =
      _renderer->m_config->GetIntWithDefault("buffers_count", 0);
  for (int i = 0; i < buffers_count; ++i) {
    auto prefix = string(1, char('A' + i));
    auto numChannels =
        _renderer->m_config->GetIntWithDefault(prefix + "_channels_count", 0);
    auto scale =
        _renderer->m_config->GetFloatWithDefault(prefix + "_scale", 1.0f);
    auto filter =
        Texture::QueryFilter(_renderer->m_config->GetStringWithDefault(
            prefix + "_filter", "linear"));
    auto warp = Texture::QueryWarp(
        _renderer->m_config->GetStringWithDefault(prefix + "_warp", "repeat"));

    if (scale < 1.0) {
      geometry =
          new ShaderToyGeometry(_width * scale, _height * scale, _x0, _y0);
    }
    m_frameBuffers.push_back(
        new ShaderToyFrameBuffer(geometry, 1.0f, numChannels, filter, warp));
  }
#if VERBOSE_OUTPUT
...
ShaderToy::ShaderToy(DuEngine* _renderer)
    : ShaderToy(_renderer, _renderer->m_window->width,
                _renderer->m_window->height, 0, 0) {}
...
void ShaderToy::recompile(DuEngine* _renderer) {
  ...
  for (int buffer = 0; buffer < 1 + this->getNumFrameBuffers(); ++buffer) {
    ...
    auto channels_count =
        m_config->GetIntWithDefault(prefix + "channels_count", 0);

    paras.uniformAppendix = "";
    for (int i = 0; i < channels_count; ++i) {
      auto iPrefix = prefix + "iChannel" + to_string(i) + "_";
      auto type =
          toLower(m_config->GetStringWithDefault(iPrefix + "type", "unknown"));
      ...
      paras.uniformAppendix += "uniform " + Texture::QuerySampler(textureType) +
                               " iChannel" + to_string(i) + "; \n";
    }
    ...
    for (int i = 0; i < channels_count; ++i) {
      ...
      Texture::QueryFileNameByType(type, fileName, m_path->getPresetPath());
      auto textureType = Texture::QueryType(type);
      auto textureFilter = Texture::QueryFilter(
          m_config->GetStringWithDefault(iPrefix + "filter", "mipmap"));
      auto textureWarp = Texture::QueryWarp(
          m_config->GetStringWithDefault(iPrefix + "wrap", "repeat"));
      ...
      switch (textureType) {
        ...
        case TextureType::RGB:
          t = m_textureManager->addTexture2D(fileName, vFlip, textureFilter,
                                             textureWarp);
          break;
        case TextureType::VideoFile:
          t = m_textureManager->addVideoFile(fileName, vFlip, textureFilter,
                                             textureWarp);
          break;
        ...
        case TextureType::VideoSequence:
          t = m_textureManager->addVideoSequence(
              fileName, fps, startFrame, endFrame, textureFilter, textureWarp);
          break;
        case TextureType::SH:
          t = m_textureManager->addSphericalHarmonics(fileName, fps, startFrame,
                                                      endFrame, numBands);
          break;
        ...
        case TextureType::CubeMap:
          vFlip = m_config->GetBoolWithDefault(iPrefix + "vflip", false);
          textureWarp = Texture::QueryWarp(
              m_config->GetStringWithDefault(iPrefix + "wrap", "clamp"));
          t = m_textureManager->addTextureCubeMap(fileName, vFlip,
                                                  textureFilter, textureWarp);
          break;
        case TextureType::LightField:
          ...
          t = m_textureManager->addTextureLightField(
              fileName, rows, cols, textureFilter, textureWarp);
          break;
        case TextureType::Bin3D:
          t = m_textureManager->addTexture3D(fileName, textureFilter,
                                             textureWarp);
          break;
...
#if DEBUG_MULTIPASS
          debug("Buffer " + to_string(buffer) + to_string(i) + " bind with " +
                to_string(bufferID) + ", whose texture ID is " +
                to_string(bindedFbo->getTextureID()));
#endif
          break;
      }
    ...
    // buffer of vector 2
    auto vec2_buffers_count =
        m_config->GetIntWithDefault("vec2_buffers_count", 0);
    uniforms->intVec2Buffers(vec2_buffers_count);

    for (int i = 0; i < vec2_buffers_count; ++i) {
      auto fileName =
          m_path->getResource("vec2_buffers" + to_string(i) + "_file");
      uniforms->bindVec2Buffer(i, fileName);
    }

    // transfer function
    auto transfer_function_count =
        m_config->GetIntWithDefault("transfer_function_count", 0);
  ...
  info("Initialization of the scene costs: " +
       to_string(float(clock() - begin_time) / CLOCKS_PER_SEC) + " s");
}
...
int ShaderToy::getNumFrameBuffers() { return (int)m_frameBuffers.size(); }
